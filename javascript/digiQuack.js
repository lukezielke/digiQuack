function convert() {
	...
	for (line = 0; line < duckyScript.length; line++) {
		...
		if (duckyScript[line].slice(0, 4) == "REM ") {
			...
			previousStatement = duckyScript[line].replace("REM_BLOCK", "\t/*")
		} else if (duckyScript[line].slice(0, 8) == "END_REM") {
			previousStatement = duckyScript[line].replace("END_REM", "\t*/")
		} else if (duckyScript[line].slice(0, 5) == "DELAY") {
			previousStatement = "\tDigiKeyboard.delay(" + parseInt(duckyScript[line].slice(6)) + ");";
		} else if (duckyScript[line].slice(0, 7) == "STRING ") {
